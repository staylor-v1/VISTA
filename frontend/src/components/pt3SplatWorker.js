...
function parsePly(text) {
  ...
  return { positions: new Float32Array(positions), colors: new Float32Array(colors), layers: [{ id: 'baked', label: 'Baked splats', count: positions.length / 3, visible: true, opacity: 1 }] };
}
...
function parseJson(text) {
  ...
  return { positions, colors, layers: [...layerCounts].map(([id, count]) => ({ id, label: id, count, visible: true, opacity: 1 })), metadata: payload.metadata || {} };
}
...
self.onmessage = async (event) => {
  ...
  try {
    ...
    self.postMessage({ id, ok: true, ...parsed }, [parsed.positions.buffer, parsed.colors.buffer]);
  } catch (error) {

...
function Invoke-ConnectivityChecks {
    ...
    # Build the bash probe script in a variable before passing it to `bash -c`.
    # This avoids parsing edge cases seen in Windows PowerShell 5.1 when inline
    # here-strings are used directly in command argument position.
    $backendProbeScript = @'
set -e
cd /app/backend
python - <<"PY"
...
'@
    ...
    Invoke-Compose exec -T backend-dev bash -c $backendProbeScript

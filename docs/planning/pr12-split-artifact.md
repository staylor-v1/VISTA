...
1. **PR-12 milestone 4 step 2 — frontend ingest-validation action + discrepancy summary baseline**
   - Scope:
    - Add Project Data action in `InspectionWorkbenchPanel` to run ingest validation against existing project records.
    - Invoke `POST /api/projects/{project_id}/ingest` with bounded synthetic payloads derived from visible batch/part context.
    - Surface ingest counters/discrepancy totals inline after action completion with explicit success/error alerts.
    - Extend existing RTL progressive synthetic-user matrix (`basic`, `intermediate`, `advanced`) across `PT1/PT2/PT3` to validate ingest action wiring.
   - Files:
   ...
   - Automated coverage:
    - Existing frontend Jest/RTL framework:
      - progressive synthetic-user matrix across `PT1/PT2/PT3`,
      - ingest action request validation (`POST /ingest`) and inline result rendering checks.
    - Existing Playwright E2E + screenshot analytics workflow:
      - cross-type (`PT1/PT2/PT3`) progressive-user run asserts ingest action in end-to-end workflows,
      - visual analytics write-up captured in `docs/planning/pr12-ingest-screenshot-analysis.md` from runtime screenshot artifact (binary intentionally not committed).

2. **PR-12 milestone 4 step 1 — backend bulk-ingest discrepancy counters baseline**
   - Scope:
...
3. **PR-12 milestone 3 step 2 — frontend panel-layout controls + persistence wiring**
   - Scope:
...
4. **PR-12 milestone 3 step 1 — workspace-state panel-layout hardening baseline**
   - Scope:
...
5. **PR-12 milestone 2 step 2 — configurable hotkeys runtime binding in inspection workbench**
   - Scope:
...
6. **PR-12 milestone 2 step 1 — configurable hotkeys storage + validation baseline**
   - Scope:
...
- Ingest discrepancy counters/validation APIs follow-up: dashboard-level discrepancy rollups and operator triage workflow.
